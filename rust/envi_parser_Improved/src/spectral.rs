...
pub fn extract_spectral_profile(
    ...
    let bpp = header.bytes_per_pixel as u32;
    let samples = header.samples;
    let lines = header.lines;
    let bands = header.bands;
    let header_offset = header.header_offset as u32;

    // 坐标边界检查
    if x >= samples || y >= lines {
    ...
    match header.interleave {
        Interleave::Bsq => {
            ...
            for i in 0..bands {
                ...
                let pixel_offset_in_band = (y * samples + x) * bpp;
                let absolute_pixel_offset = header_offset + band_offset + pixel_offset_in_band;

                // 转换为数据块内的相对偏移
                let start_in_chunk = absolute_pixel_offset - chunk_start_offset_in_file;
                let end_in_chunk = start_in_chunk + bpp;

                if end_in_chunk as usize > chunk_data.len() {
                    return Err(EnviError::InvalidFieldValue {
                        field: "chunk_data".to_string(),
                        value: format!("BSQ read for band {} at offset {} exceeds chunk length", i, end_in_chunk),
                    });
                }
                
                let pixel_bytes = &chunk_data[start_in_chunk as usize..end_in_chunk as usize];
                // 使用 reader 内部的辅助函数
                let value = reader.bytes_to_f32(pixel_bytes)
                    .map_err(|js_err| EnviError::InvalidFieldValue { 
        ...
        Interleave::Bil => {
            ...
            for i in 0..bands {
                let line_start_offset = y * bytes_per_full_line;
                let band_start_in_line_offset = i * bytes_per_line_per_band;
                let pixel_start_in_band_offset = x * bpp;
                ...
                if end_in_chunk as usize > chunk_data.len() {
                    return Err(EnviError::InvalidFieldValue {
                        field: "chunk_data".to_string(),
                        value: format!("BIL read for band {} at offset {} exceeds chunk length", i, end_in_chunk),
                    });
                }
        ...
        Interleave::Bip => {
            ...
            let pixel_index = y * samples + x; // 第几个像素
            let pixel_group_start_offset = pixel_index * bytes_per_full_pixel;

            for i in 0..bands {
                let band_offset_in_pixel = i * bpp;
                let absolute_pixel_offset = header_offset + pixel_group_start_offset + band_offset_in_pixel;
                ...
                if end_in_chunk as usize > chunk_data.len() {
                    return Err(EnviError::InvalidFieldValue {
                        field: "chunk_data".to_string(),
                        value: format!("BIP read for band {} at offset {} exceeds chunk length", i, end_in_chunk),
                    });
                }
...
pub fn pair_with_wavelengths(
    header: &EnviHeader,
    spectral_curve: Vec<f32>,
) -> Vec<SpectralPoint> {
    ...
    if let Some(wavelengths) = &header.wavelength {
        if wavelengths.len() == spectral_curve.len() {
            // 将波长和值打包在一起
            return wavelengths.iter()
                .zip(spectral_curve.iter())
                .map(|(&w, &v)| SpectralPoint { wavelength: w, value: v })
                .collect();
        }
    }
    ...
    spectral_curve.iter()
        .enumerate()
        .map(|(i, &v)| SpectralPoint { wavelength: (i + 1) as f64, value: v })
        .collect()
}